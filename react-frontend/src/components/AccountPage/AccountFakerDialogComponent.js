...
import accountFakerFactory from "./accountFakerFactory";
...
const AccountFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = accountFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("account").create(elem));
...
export default connect(null, mapDispatch)(AccountFakerDialogComponent);
