...
import gameFakerFactory from "./gameFakerFactory";
...
const GameFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = gameFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("game").create(elem));
...
export default connect(null, mapDispatch)(GameFakerDialogComponent);
