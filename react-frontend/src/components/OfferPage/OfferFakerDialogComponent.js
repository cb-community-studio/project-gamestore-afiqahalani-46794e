...
import offerFakerFactory from "./offerFakerFactory";
...
const OfferFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = offerFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("offer").create(elem));
...
export default connect(null, mapDispatch)(OfferFakerDialogComponent);
